    ...
    public Property SameSeedProducesSameRooms()
    {
        return Prop.ForAll(
            ...
            (config, seed) =>
            {
                ...
                var partitions = splitter.Split(config.MapWidth, config.MapHeight, config.GridRows, config.GridColumns);
                ...
                var result1 = roomGen.GenerateRooms(partitions, config, grid1, rng1);
                var result2 = roomGen.GenerateRooms(partitions, config, grid2, rng2);
