...
public sealed class RegenerationLoop
{
    ...
    /// </summary>
    public static GenerationResult Execute(GenerationConfig config)
    {
        ...
        for (int attempt = 0; attempt <= MaxRetries; attempt++)
        {
            ...
            _ = new RoomGenerator();
            var roomResult = RoomGenerator.GenerateRoomsBSP(clamped, grid, rng);

            // 3. Corridor connection via MST + extra edges
            _ = new CorridorConnector();
            var corridorResult = CorridorConnector.Connect(roomResult.Rooms, roomResult.Metadata, clamped, grid, rng);

            // 4. Entrance marking
            _ = new EntranceMarker();
            EntranceMarker.MarkEntrances(grid, roomResult.Rooms, corridorResult.Corridors);

            // 5. Spawn placement
            _ = new SpawnPlacer();
            var spawnResult = SpawnPlacer.PlaceSpawns(grid, roomResult.Rooms, roomResult.Metadata, corridorResult.Corridors, clamped, rng);

            if (!spawnResult.Success)
                continue; // Retry with new seed

            // 6. Connectivity validation
            _ = new ConnectivityValidator();
            bool connected = ConnectivityValidator.Validate(grid, roomResult.Metadata, spawnResult.PlayerSpawn);
